...
ctr=1
rm -f dis_calc/resultparamfile.txt
touch dis_calc/resultparamfile.txt
phenos=()
#scores=()
disease_names=()
...
while read line; do
    ...
    echo -e $disease_name'\t'dis_calc/scores_$ctr.profile >>dis_calc/resultparamfile.txt
    let ctr++
...
if [[ $dataset == $backgr_dataset ]]; then
    bash dis_calc/run_simple_prs_2.sh $backgr_dataset $dataset $scores 1 2 3 f
else
    bash dis_calc/run_simple_prs_2.sh $backgr_dataset $dataset $scores 1 2 3 t
fi
# get patient ID from VCF
patient_id_new=$(python3 dis_calc/get_id_from_vcf.py $dataset)

#bash dis_calc/run_simple_prs_2.sh $backgr_dataset $dataset $scores 1 2 3 3 
rm -f dis_calc/results_for_script.txt
touch dis_calc/results_for_script.txt
ctr=1
#echo "${phenos[*]}"
#echo ${phenos[0]}
#echo ${phenos[1]}
...
do
    if [[ $predict == "f" ]]; then
        ...
        tmp=$(python3 dis_calc/get_relative_risk.py dis_calc/scores_$ctr.profile $patient_id_new dis_calc/results_and_phenos$ctr.txt)
        ...
        tmp=$(bash dis_calc/get_absolute_risk.sh dis_calc/scores_$ctr.profile ${phenos[ctr]} $patient_id_new dis_calc/results_and_phenos$ctr.txt)
        risk=$(echo $tmp | awk '{print $2}')
        percentile=$(echo $tmp | awk '{print $3}')
    fi
    echo -e $dis"\t"$risk"\t"$percentile"\t"dis_calc/results_and_phenos$ctr.txt >>dis_calc/results_for_script.txt
    let ctr++
done

## write report
python3 dis_calc/make_prs_report.py "easy" dis_calc/results_for_script.txt $patient_id_new "your_prs_report.pdf" $predict
